...
if selected == "Dashboard":
    ...
    with l:
        ...
        # Dynamic credit calculation based on last Nitrogen input
    ...
    with right:
        ...
        if user_input:
            ...
            with st.spinner("KrishiMitra is typing..."):
                prompt = f"Expert Indian Agronomist Mode. Current Diagnosis: {st.session_state.detected_disease}. Question: {user_input}. Provide 3 specific steps for recovery."
...
# --- 6. CROP PLANNER PAGE ---
    ...
    st.session_state['last_n'] = n # Updates Dashboard credits
    
    ph = st.slider("Soil pH Level", 4.0, 10.0, 6.5)
    
    if st.button("🔮 Forecast Best Crop"):
        if crop_model:
            # Features order must match your model's training order (N, P, K, temp, hum, ph, rain)
